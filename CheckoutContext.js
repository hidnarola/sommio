...
function CheckoutProvider({ cartId: initialCartId, children, ...props  }) {
  ...
  const {rate,shippingProvider,cartItems,shipping_address,customer} = useContext(CartContext)
  const [state, dispatch] = useReducer(reducer, initialState);
  console.log('shipping_providers => ',shippingProvider);
  ...
  async function checkout(
    ...
    const createCustomer = customerDetails && customerDetails.password
    let customerId
  ...
  console.log('customItemShipping => ',customItemShipping);

    if (createCustomer) {
      console.log('createCustomer sssss=> ',createCustomer);
    ...
    const { data: order } = await moltin.post(`carts/${cartId}/checkout`, {
      // ...(createCustomer ? { customer: { id: customerId } } : { cust }),
      customer,
      shipping_address,
      billing_address,
      shipping_cost,
  ...
  function paymentData(paymentDetails){
    console.log('paymentDetails,data => ',paymentDetails );
    dispatch({type: PAYMENT , payload: paymentDetails})
  }
