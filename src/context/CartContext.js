...
export default function reducer(state, action) {
  ...
  switch (action.type) {
    case SET_CART:
      ...
      const count = cartItems.reduce((sum, {type, quantity}) => type === "cart_item" && sum + quantity, 0);
    ...
    console.log("subTotal==>", subTotal,meta.display_price.without_tax.formatted);
...
function CartProvider({
 ...
 async function addToCart(id, quantity,size,weight,color,subTotal,rate) {
  ...
  console.log('items => ',items);
    ...
    let payload = await axios.post("https://sandbox-api.postmen.com/v3/rates",
      data,
...
async function shippingCost(convertedRates ,shipping_provider){
  console.log(' convertedRates, shipping_provider=> ',convertedRates, shipping_provider);
  dispatch({type: SET_SELCETED_RATES , payload:{ convertedRates, shipping_provider }})
}
