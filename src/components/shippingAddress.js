import React, { useContext, useState } from 'react'
import { Form, Field } from 'react-final-form'
import Input from '../components/Input'
// import validation from '../validation/checkout'

import { CartContext } from '../context'
import validation from '../validation/checkout'

import AddressFields from '../components/AddressFields'
import { LocationProvider } from '@reach/router'

const initialValues = {
  billingIsShipping: true,
  createCustomer: false,
  customer: {
    marketing_opt_in: false
  }
}

function ShippingAddress({ isCompleted, toggleEditable }) {
  const { cartItems, shippingCostCalculate, shipping_address, customerDetails } = useContext(
    CartContext
  )
  const [isSubmit, setSubmit] = useState(true)

  const handleShippingCost = values => {
    toggleEditable(true)
    shippingCostCalculate(values, cartItems)
  }

  return isCompleted ? (
    <>
    <div className="shipping-boxs">
      <h2 className="text-black font-medium leading-loose p-0 mb-3"><span>1</span>SHIPPING & BILLING</h2>
      <div className="mb-10">
        <h4 className="mb-3">Email Address</h4>
        <p>{customerDetails && customerDetails.email}</p>
      </div>
      <div className="mb-10">
        <h4 className="mb-3">Shipping Address</h4>
        <p className="mb-1">{shipping_address && shipping_address.first_name} {shipping_address && shipping_address.last_name}</p>
        <p className="mb-1">{ shipping_address && shipping_address.line1}</p>
        <p className="mb-1">{shipping_address && shipping_address.city}</p>
        <p>{shipping_address && shipping_address.county}</p>
      </div>
    </div>
    <div className="submit_btn">
      <button className="btn btn-outline-secondary" onClick={() => toggleEditable(false)}>Edit</button>
    </div>
    </>
  ) : (
    <Form onSubmit={handleShippingCost} validate={validation}>
      {({ handleSubmit, pristine, invalid, value }) => {
        return (
          <form onSubmit={handleSubmit} id="shipping_form">
            <div className="shipping-boxs">
              <h2 className="text-black font-medium leading-loose p-0 mb-3 pt-6 pb-3 border-b border-grey-light">
                <span>1</span>CONTACT INFORMATION
              </h2>
              <div className="frm_grp">
                <Input type="email" name="customer.email" label="Email" />
              </div>
              <h2 className="text-black font-medium leading-loose p-0 mb-3 pt-6 pb-3 border-b border-grey-light">
                SHIPPING & BILLING
              </h2>
              <AddressFields type="shipping_address" />
            </div>
            <div className="submit_btn">
              <button type="submit" disabled={pristine || invalid }>
                Submit
              </button>
            </div>
          </form>
        )
      }}
    </Form>
  )
}

export default ShippingAddress
